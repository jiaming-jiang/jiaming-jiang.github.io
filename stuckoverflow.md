...
Tags, also known as "video tags," are keywords or phrases that provide context to a YouTube video that give users a ‘heads-up’ on what the video is about. These tags are considered a key factor in YouTube's search algorithm as they assist users in discovering and organizing content, as well as improving a video's ranking in search results. <br><br>
However, after YouTube decided to hide video tags in public pages in the attempt to mitigate tag abuse and misuse, a discussion of the relevance of tags arose in the YouTube community. People started to question whether tags are still important as more weights are now given to the video’s title, thumbnails, and description when being sorted by YouTube’s algorithm. Do tags really not matter anymore? <br><br>
Nevertheless, we are more interested in finding out:
 <br>
...
<div style="text-align: justify">There are many ways to describe the content of a video, people are likely to use different tags to describe similar content. We might investigate how long tags are used in this dynamic environment.
...
<div style="text-align: justify"> 
As a next analysis, we investigated how long these tags remain popular before starting to lose interest. We visualized the histogram of the number of months the Z-scores of remain positive for all popular tags:
</div>
...
From this bipartite graph, with a projection, we can even observe the network between tags where the weigths represent how many videos two tags were used together.
...
<div style="text-align: justify">We indeed see an increase in all three centrality measures in the popularity month. So, popular tags do get more important in the tag network when they are popular.
...
To check if the videos that do not contain these tag pairs are misused, we sample 20% of the videos tagged with “trump” and perform manual checks to determine if the usage of tags in these videos are appropriate. 
...
Among all randomly selected videos, 59.2% were labeled as misuse, 19.6% as Related and 21.2% as N/A. If we exclude N/A, 75.2% of the videos would fall under the category of misuse. The categories that had the most misuse are:
...
<div style="text-align: justify"> The graph shows a slight drop in October 2016 and January 2017, but peaked in November and December 2016; we can assume that more "trump" tags were misused in these 2 months. It's time to put our approach to the test with some different tags. The World Cup is a huge, global event, so let's see if we can find any examples of misuse. We'll sample videos from 2018 using the aforementioned approach and check out a few random videos that are labeled as misuse. Get ready for some tag-tastic data!
</div>

{% include world_cup_examples.html %}

<div style="text-align: justify"> After a quick manual inspection, we found that most of the titles are unrelated, indeed. However, we did mistakenly label a few titles. Even though it did not come home, video with title **"IT'S COMING HOME !!!!!! (Runescape Song)"** tricked us. Oops!
...
<div style="text-align: justify">Tag misuse on YouTube has been a longstanding issue that can impact the ability of users to find relevant content and can also lead to abuse of the tagging system. Indeed, based on the data analyzed, it has been found that tags on YouTube can be misused in order to manipulate the visibility of a video or to exploit trending topics. The average number of tags per video has increased over time, and certain tags are used more frequently together than others. Some tags are also more likely to be used with the tag "trump" than others. Additionally, some users have been found to misuse tags on unrelated video categories in order to exploit trending topics.</div>

<div style="text-align: justify">Given these findings, it can be concluded that tag misuse is a significant issue on YouTube. The platform should continue to work to combat this problem in order to ensure that users are able to find relevant content and to prevent abuse of the tagging system. It may also be beneficial for users to be aware of the potential for tag misuse and to carefully consider the tags they use in order to accurately and appropriately represent the content of their videos.</div>